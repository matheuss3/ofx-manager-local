    ...
    @if (store.log().length) {
      <div class="log-panel">
        <button class="log-toggle" (click)="open.set(!open())">
          ...
          <span class="log-chevron" [class.open]="open()">▼</span>
        </button>
        @if (open()) {
                ...
                <div class="log-entry" [class.undone]="entry.undone">
                  ...
                  <div class="log-info">
                    ...
                    <div class="log-meta">
                      ...
                      @if (entry.undone) { <span class="text-green"> · ↩ desfeito</span> }
                    </div>
                  </div>
                  @if (!entry.undone) {
                    <button class="log-undo" (click)="store.undoLog(entry.id)">↩ Desfazer</button>
                  }
                </div>
              }
            </div>
